...
module.exports = {
	get: (req, res, next) => {
		...
		db.query(`
			SELECT disponibility.id_teacher, name AS name_teacher, payment_per_hour, detail.advisory_score,
				hour_start, hour_end
			FROM prest.disponibility AS disponibility
				INNER JOIN prest.user AS teacher ON teacher.id_user = disponibility.id_teacher
				INNER JOIN prest.teacher AS detail ON detail.id_teacher = disponibility.id_teacher
